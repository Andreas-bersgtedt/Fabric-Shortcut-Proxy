...
if [[ $ARG_SKIP_INSTALL -eq 1 ]]; then
  ...
  step "Installing project dependencies, database drivers, object-store readers, Azure (Blob + Key Vault), and OneLake (Open Mirroring) from pyproject.toml"
  "$VENV_PYTHON" -m pip install -e '.[drivers,objectstore,azureblob,keyvault,onelake]' -e ./enterprise --quiet
