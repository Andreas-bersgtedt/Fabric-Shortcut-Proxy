...
The base uses PostgreSQL because `Dockerfile.python` installs the `postgres` extra by default. Set the `FSP_EXTRAS` build argument for another packaged driver set. SQL Server containers also require the Microsoft ODBC driver at the operating-system layer.
